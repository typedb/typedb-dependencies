...
DOCKER_ORG=typedb
DOCKER_REPO=amazonlinux2-ci
PLATFORMS=("amd64" "arm64") # Update `docker manifest` commands below if it's changed

for ARCH in "${PLATFORMS[@]}"; do
  ASSEMBLE_TARGET="//images/docker/amazonlinux2-3x-ci:assemble-docker-${ARCH}"
  DEPLOY_TARGET="//images/docker/amazonlinux2-3x-ci:deploy-docker-${ARCH}"
